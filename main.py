...
def detect_available_llm_provider():
    ...
    Returns: (provider, model_name, has_key)
    """
    if os.getenv("GEMINI_API_KEY"):
        return "google", "gemini-2.0-flash", True
    else:
        return "huggingface", "gpt2", False
...
def main():
    ...
    parser.add_argument(
        "--llm",
        default=None,
        choices=["ollama", "huggingface", "ollama", "google"],
    ...
    if args.llm is None or args.llm_model is None:
        detected_provider, detected_model, has_key = detect_available_llm_provider()
        if args.llm is None:
            args.llm = detected_provider
        if args.llm_model is None:
            args.llm_model = detected_model
        else:
            print("No API keys found, using HuggingFace")
